...
class RequestsService:
    ...
    async def get_requests(self, current_user: User, pagination: PaginationParams):
        ...
        if current_user.role == UserRole.ADMIN:
            ...
            response = await self.requests_repo.get_subordinates_requests(current_user.id, pagination)
        elif current_user.role == UserRole.USER:
            response = await self.requests_repo.get_user_requests(current_user.id, pagination)
        else:
            raise HTTPException(status_code=403, detail="Only admin and managers can access this resource.")
        return response

    async def get_user_requests(self, current_user: User, user_id: int, pagination: PaginationParams):

        if current_user.id == user_id:
            response = await self.requests_repo.get_user_requests(user_id, pagination)

...
class UsersRepository(SQLAlchemyRepository):
    ...
    async def get_subordinates(self, manager_id: int, pagination) -> dict:
        async with async_session_maker() as session:
            query = select(self.model).where(self.model.manager_id == manager_id)
            query = pagination.apply(query, self.model)

            total_query = select(func.count()).select_from(self.model).where(self.model.manager_id == manager_id)
